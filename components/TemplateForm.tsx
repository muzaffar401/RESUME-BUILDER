...
import { uploadThumbnail } from "@/lib/supabase/client"
...
const TemplateForm = () => {
    ...
    return (
        <form className="flex flex-col gap-8" onSubmit={handleSubmit(handleTemplateForm)}>
            <Input label="Name" {...register("name", { required: true })} placeholder="Enter template name" />
            ...
            <div>
                ...
                <ReactCodeMirror id="html" value={html} onChange={(val) => setHtml(val)} />
            </div>
            <div className="flex gap-4">
                <label htmlFor="checkbox">Is it Paid</label>
                <input id="checkbox" type="checkbox" {...register("paid")}/>
            </div>
            <button disabled={loading} className="bg-emerald-400 text-white py-3 px-10 rounded-lg mt-4 hover:scale-105 transition-all ease-in-out self-start">{loading?<span className="animate-pulse">Submitting...</span>:"Submit"}</button>
        </form>
    )
}
